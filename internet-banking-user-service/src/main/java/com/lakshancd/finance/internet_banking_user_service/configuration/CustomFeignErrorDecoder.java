...
public class CustomFeignErrorDecoder implements ErrorDecoder {
    ...
    private SimpleBankingGlobalException extractBankingCoreGlobalException(Response response) {
        ...
        Reader reader = null;
        //capture the response body and convert it to SimpleBankingGlobalException
        try{
            reader = response.body().asReader(StandardCharsets.UTF_8);
            String result = IOUtils.toString(reader);
            ObjectMapper objectMapper = new ObjectMapper();
            objectMapper.disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);
            exceptionMessage = objectMapper.readValue(result , SimpleBankingGlobalException.class);
        } catch (IOException e){
